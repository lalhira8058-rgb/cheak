...
async def check_card(card, site, proxy):
    try:
        ...
        # Price mismatch = site alive but product price changed — retry with different site
        if 'merchandise_expected_price_mismatch' in response_msg.lower() or 'price_mismatch' in response_msg.lower():
...
async def test_site_with_price(site, proxy):
    ...
    try:
        ...
        price_value = get_price_from_response(raw)
        # Price mismatch means site is alive, just product price changed — mark price as 0.0 (unknown)
        if 'merchandise_expected_price_mismatch' in response_msg.lower() or 'price_mismatch' in response_msg.lower():
            # Site is alive but we don't know the real price — store 0.0 so it falls into lowest filter
...
async def get_all_sites(event):
    ...
    sites_data = await load_sites_with_price()
    price_map = {s['url']: s.get('price', 0.0) for s in sites_data}
    if not current_sites:
        await event.reply(premium_emoji("❌ Nᴏ sɪᴛᴇs ɪɴ sɪᴛᴇs.ᴛxᴛ"), parse_mode='html')
        return
    if len(current_sites) <= 30:
        site_list = "\n".join([f"{i+1}. <code>{s}</code> — ${price_map.get(s, 0.0):.2f}" for i, s in enumerate(current_sites)])
        await event.reply(premium_emoji(f"🌐 <b>Aʟʟ Sɪᴛᴇs ({len(current_sites)})</b>\n\n{site_list}"), parse_mode='html')
    else:
        timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
        filename = f"sites_{user_id}_{timestamp}.txt"
        async with aiofiles.open(filename, 'w') as f:
            for i, site in enumerate(current_sites):
                price = price_map.get(site, 0.0)
                await f.write(f"{i+1}. {site} | ${price:.2f}\n")
        await event.reply(
            premium_emoji(f"🌐 <b>Aʟʟ Sɪᴛᴇs ({len(current_sites)})</b>\n\nFɪʟᴇ ᴀᴛᴛᴀᴄʜᴇᴅ ʙᴇʟᴏᴡ."),
            file=filename,
            parse_mode='html'
        )
        try:
        ...
        for i in range(0, len(sites), batch_size):
            ...
            for res in results:
                if res['status'] == 'alive':
                    ...
                    dead_sites.append(res['site'])
            await status_msg.edit(premium_emoji(f"🔄 Cʜᴇᴄᴋɪɴɢ sɪᴛᴇs...\n\nCʜᴇᴄᴋᴇᴅ: {len(alive_sites) + len(dead_sites)}/{len(sites)}\n✅ Aʟɪᴠᴇ: {len(alive_sites)}\n❌ Dᴇᴀᴅ: {len(dead_sites)}"), parse_mode='html')
        # Merge with existing sites (avoid duplicates) — normalize URLs for comparison
        existing_sites = load_sites()
        existing_sites_with_price = await load_sites_with_price()
        # Normalize: strip trailing slash and lowercase for dedup
        def norm(u): return u.rstrip('/').lower()
        existing_norm = set(norm(s) for s in existing_sites)
        
        new_alive = [s for s in alive_sites if norm(s) not in existing_norm]
        merged_sites = existing_sites + new_alive
        merged_sites_with_price = existing_sites_with_price + [s for s in sites_with_price if norm(s['url']) not in existing_norm]
        
        async with aiofiles.open(SITES_FILE, 'w') as f:
            for site in merged_sites:
                await f.write(f"{site}\n")
        await save_sites_with_price(merged_sites_with_price)
        result_text = f"""✅ <b>Sɪᴛᴇs ᴜᴘᴅᴀᴛᴇᴅ sᴜᴄᴄᴇssғᴜʟʟʏ!</b>

📊 Tᴏᴛᴀʟ sɪᴛᴇs ʀᴇᴄᴇɪᴠᴇᴅ: {len(sites)}
✅ Nᴇᴡ Aʟɪᴠᴇ (ᴀᴅᴅᴇᴅ): {len(new_alive)}
⚠️ Dᴜᴘʟɪᴄᴀᴛᴇs (sᴋɪᴘᴘᴇᴅ): {len(alive_sites) - len(new_alive)}
❌ Dᴇᴀᴅ (ɪɢɴᴏʀᴇᴅ): {len(dead_sites)}
🌐 Tᴏᴛᴀʟ ɪɴ ᴅʙ: {len(merged_sites)}

🌐 <b>Aᴅᴅᴇᴅ sɪᴛᴇs:</b>
{chr(10).join([f"• {s}" for s in new_alive[:5]])}{'...' if len(new_alive) > 5 else ''}"""
        await status_msg.edit(premium_emoji(result_text), parse_mode='html')
    except Exception as e:
        await status_msg.edit(premium_emoji(f"❌ Eʀʀᴏʀ: {e}"), parse_mode='html')
...
async def split_custom_callback(event):
    ...
    async def get_custom_size(msg_event):
        try:
            ...
            if chunk_size > 15000:
                await msg_event.reply(premium_emoji("❌ Mᴀxɪᴍᴜᴍ 15000 ᴄᴀʀᴅs ᴘᴇʀ ғɪʟᴇ!"), parse_mode='html')
                return
...
async def merge_handler(event):
    ...
    # Skip command-like text triggers
    if event.text and event.text.upper() in ["MERGE", "+5M", "CANCELM", "COLLECT", "+5 MIN", "CANCEL"]:
        return

    # Handle file sent directly (no reply needed)
    target_msg = event
    if event.reply_to_msg_id:
        target_msg = await event.get_reply_message()

    # Accept file from the event itself or from replied message
    file_msg = None
    if event.file and event.file.name and event.file.name.endswith('.txt'):
        file_msg = event
    elif event.reply_to_msg_id:
        replied = await event.get_reply_message()
        if replied.file and replied.file.name and replied.file.name.endswith('.txt'):
            file_msg = replied
